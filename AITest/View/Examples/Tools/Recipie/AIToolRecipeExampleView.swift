    ...
    var body: some View {
		Form {
			...
			if let answer = answer, !isAnswerLoading {
				RecipeItemImageHeaderView(recipe: answer)
		...
		.navigationTitle(PerformanceMetricType.recipeWithTool.navigationTitle)
		.overlay {
			GenerateAnswerButtonView {
				Task {
					await PerformanceMeasurer.measureAndSave(
						metricType: .recipeWithTool,
						task: {
		...
		.sheet(isPresented: $isShowingChart) {
			ExecutionTimeChartView(
				userDefaultsKey: PerformanceMetricType.recipeWithTool.userDefaultsKey,
				navigationTitle: PerformanceMetricType.recipeWithTool.navigationTitle,
				unitName: PerformanceMetricType.recipeWithTool.unitName
			)
...
#Preview {
	AIToolRecipeExampleView(answer: .japaneseCurry)
}
